...
void WebServerService::handleRoot() {
  ...
  auto buildPinSelect = [&](String name, String label, int currentValue) {
      ...
      // Board-aware pin range. C3: 0..21. C6: 0..30.
      // On C6 we still surface strapping/flash/USB pins for advanced users,
      // but mark them so beginners don't pick them by accident.
      for (int p = BoardPins::MIN_GPIO_PIN; p <= BoardPins::MAX_GPIO_PIN; p++) {
          bool blocked = isPinBlocked(p);
          String labelStr = "GPIO " + String(p);
          if (blocked) {
            labelStr += " ⚠️";
          }
          // Add silkscreen label (D0..D10) when present
          constexpr int silk_count = sizeof(BoardPins::SILKSCREEN) / sizeof(BoardPins::SILKSCREEN[0]);
          if (p >= 0 && p < silk_count && BoardPins::SILKSCREEN[p][0] != '\0') {
            labelStr += " (" + String(BoardPins::SILKSCREEN[p]) + ")";
          }
          out += "<option value='" + String(p) + "'" +
                 (p == currentValue ? " selected" : "") + ">" + labelStr + "</option>";
      }
      out += "</select>";
      return out;
  };
