...
// Active pin defaults (selected at build time via TINYTOSH_BOARD)
const int DEFAULT_TOUCH_PIN = BoardPins::DEFAULT_TOUCH_PIN;
const int DEFAULT_SDA_PIN   = BoardPins::DEFAULT_SDA_PIN;
const int DEFAULT_SCL_PIN   = BoardPins::DEFAULT_SCL_PIN;
const int MIN_GPIO_PIN      = BoardPins::MIN_GPIO_PIN;
const int MAX_GPIO_PIN      = BoardPins::MAX_GPIO_PIN;
const char* BOARD_NAME      = BoardPins::BOARD_NAME;
const char* BOARD_ID        = BoardPins::BOARD_ID;
...
void validatePins(Config& config) {
  ...
  auto claimPin = [&](int &pin, int default_pin) {
    ...
    // 2. Check for collision or blocked pin (C6 strapping/flash/USB)
    if (used_pins[pin] || isPinBlocked(pin)) {
      pin = default_pin;

      // 3. Find the absolute first available free + unblocked pin
      if (used_pins[pin] || isPinBlocked(pin)) {
        for (int i = MIN_GPIO_PIN; i <= MAX_GPIO_PIN; i++) {
          if (!used_pins[i] && !isPinBlocked(i)) {
            pin = i;
            break;
          }
        }
...
void setup() {
  ...
  // First-boot detection: if the saved config still has the C3 defaults
  // (sda_pin == 8 && scl_pin == 9) and we're targeting a different board,
  // swap in the board-specific defaults so the OLED + button work out of
  // the box without forcing the user into the web panel.
#if TINYTOSH_BOARD == XIAO_ESP32_C6
  bool looksLikeFreshFlash = (appState.config.sda_pin == BoardPins::C3_SDA_PIN &&
